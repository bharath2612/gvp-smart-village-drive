...
export function buildAmenities(ctx) {
  ...
  { const a = A.temple; const cx = a.x + a.w / 2, cz = a.y + a.h / 2;
    ...
    for (let i = 0; i < 3; i++) { const w = 70 - i * 4; sand.push(box(w, 0.5, w, i === 2 ? SAND : SAND2, { x: cx, z: cz, y: 0.25 + i * 0.5 })); }
    ...
    sand.push(box(31, 0.8, 31, SAND2, { x: cx, z: tz, y: y + 0.4 })); y += 0.8;
    ...
    sand.push(box(31.4, 0.6, 31.4, CORN, { x: cx, z: tz, y: y + 9.3 })); y += 9.6;
    const tiers = 7;
    ...
    for (let i = -2; i <= 2; i++) { const x = cx + i * (cw / 5); sand.push(cyl(0.35, 0.55, 1.4, 10, GOLD, { x, z: tz, y: y + 3.5 + 0.7 })); sand.push(ico(0.55, 1, GOLD, { x, y: y + 5.3, z: tz })); sand.push(cyl(0, 0.3, 1.2, 8, GOLD, { x, z: tz, y: y + 6.4 })); }
  ...
  add(sand, stdMat(T, T.plaster, { vertexColors: true, roughness: 0.9 }), 'temple-sandstone');
