...
export class Minimap {
  constructor(world, canvas, bigCanvas) {
    this.bounds = world.campus?.bounds || { minX: 0, minZ: 0, maxX: 3000, maxZ: 3000 }; this.extent = this.bounds.maxX - this.bounds.minX;
  ...
  bigToWorld(clientX, clientY) {
    ...
    const x = (px - this.big.ox) / this.big.k + this.bounds.minX, z = (py - this.big.oy) / this.big.k + this.bounds.minZ;
    if (x < this.bounds.minX || z < this.bounds.minZ || x > this.bounds.maxX || z > this.bounds.maxZ) return null; return { x, z };
  }
  drawBase(g, size, labels) {
    ...
    for (const r of this.world.roads) rect(r, r.kind === 'spine' ? '#c9c4b8' : r.kind === '25m' ? '#d8d3c8' : '#a9a49a');
    g.strokeStyle = '#c9a227'; g.lineWidth = Math.max(2, 3 * k); g.strokeRect(0, 0, 3000 * k, 3000 * k);
    if (this.world.campus) this.drawCampus(g, k, labels);
    if (labels) {
      ...
      g.fillText('Village gate', 1500 * k, 2960 * k);
    }
    g.restore();
  }
  drawCampus(g, k, labels) {
    ...
    for (const a of p.precincts) {
      g.save(); g.translate(a.x*k, a.z*k); g.rotate(-a.rot);
      g.fillStyle = '#426449'; g.fillRect(-85*k,-65*k,170*k,145*k); g.fillStyle = '#c0ad8a';
      g.fillRect(-80*k,-59.5*k,160*k,23*k); g.fillRect(-80*k,-36.5*k,22*k,77*k); g.fillRect(58*k,-36.5*k,22*k,77*k); g.restore();
      if (labels) { const q = campusPoint(a,0,70); g.font = `600 ${Math.max(10,16*k)}px Arial, sans-serif`; g.fillStyle = '#f2e5c6'; g.textAlign = 'center'; g.fillText(a.name,q.x*k,q.z*k); }
    }
    ...
    if(labels){ g.fillText('GVP AIRSTRIP',1100*k,3140*k); g.fillText('CAMPUS ENTRANCE',2160*k,3700*k); g.fillText('UNIVERSITY · NORTH',1500*k,-550*k); g.fillText('WEST COLLEGES',-555*k,1500*k); g.fillText('EAST CAMPUS',3560*k,1500*k); }
  }
  draw(car, rings, target) {
    ...
    const k = W / view; const kb = this.base.width / this.extent;
    ...
    g.drawImage(this.base, 0, 0, this.base.width, this.base.height, this.bounds.minX * k, this.bounds.minZ * k, this.extent * k, this.extent * k);
  ...
  drawBig(car, rings, target) {
    const c = this.bigCanvas; const g = c.getContext('2d'); const size = Math.min(c.width, c.height); const k = size / this.extent;
    g.clearRect(0, 0, c.width, c.height);
    const ox = (c.width - size) / 2, oy = (c.height - size) / 2; this.big = { canvas: c, ox, oy, k };
    g.drawImage(this.baseBig, ox, oy, size, size);
    for (const r of rings || []) { g.fillStyle = r === target ? '#ffd35a' : 'rgba(201,162,39,0.7)'; g.beginPath(); g.arc(ox + (r.x - this.bounds.minX) * k, oy + (r.z - this.bounds.minZ) * k, 7, 0, Math.PI * 2); g.fill(); }
    g.save(); g.translate(ox + (car.x - this.bounds.minX) * k, oy + (car.z - this.bounds.minZ) * k); g.rotate(car.yaw); g.fillStyle = '#ffd35a'; g.strokeStyle = '#0a0f1c'; g.lineWidth = 2;
    g.beginPath(); g.moveTo(0, -12); g.lineTo(8, 9); g.lineTo(0, 5); g.lineTo(-8, 9); g.closePath(); g.fill(); g.stroke(); g.restore();
    if (this.mark) { const mx = ox + (this.mark.x - this.bounds.minX) * k, my = oy + (this.mark.z - this.bounds.minZ) * k; g.save(); g.strokeStyle = '#ffd35a'; g.fillStyle = 'rgba(255,211,90,0.25)'; g.lineWidth = 2;
